...
int main(){
    int NM[2], contPlayer1=0, contPlayer2=0, contLine=0, contColun=0, contEnter=0, aux=0;
    int MaxPecas=0;
        ...
        for(int i=0; i< NM[0];i++){
            for (int j=0; j< NM[1];j++){ 
                if(matriz[i][j] == 1){ // percorre a matriz até achar uma das minhas peças
                    int vec[2] = {i,j};
                    verifyDia(&matriz, vec, &contEnter); // modifico diretamente o valor de contEnter(Numero de peças comidas)
                    if(contEnter > aux) aux = contEnter; //verifico qual peça minha consegue comer mais peças inimifas e coloco em aux
                }
            }
        }
        liberarMatriz(&matriz, NM);
